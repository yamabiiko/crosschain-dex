...
\smallskip
\textbf{Parameters:} Integers \texttt{windowSize}, \texttt{Delay}; Algorithms $\mathsf{Validate}$, $\mathsf{ExtendPolicy}$, $\mathsf{Blockify}$, $\mathsf{Blind}$, $\mathsf{BlindTx}$, P. Random distribution $\mathcal{X}$ \\
\smallskip
\textbf{Clock-time:} \\
\smallskip
\textbf{Variables and initialization:} The functionality intializes $\state$, $\texttt{NxtBC}$, $\buffer \gets \emptyset$ \\

\begin{pchstack}[boxed]
\begin{pcvstack}
\pseudocode{
    \mathsf{Input}(I, P)  \:\: \\[0.1\baselineskip ][\hline]
    \tau_L \gets \mathcal{G}_\mathsf{CLOCK} \\
    \pcif P \in \mathcal{P} \setminus \mathcal{H} \\
    \t \mathsf{StateUpdate} \\
    \pcif I = (\mathsf{SUBMIT}, \mathsf{sid}, \texttt{tx}) \\
    \t \mathsf{TxSubmit}(\mathsf{sid}, \texttt{tx}) \\
    \t I := (\mathsf{SUBMIT}, \mathsf{sid}, \mathsf{BlindTx}_\adv(\state, \mathcal{P} \setminus \mathcal{H}, \texttt{tx})) \\
    \pcif I = (\mathsf{READ}, \mathsf{sid}) \\
    \t \pcif P = \adv \\
    \t\t \mathsf{send}(\mathsf{Blind}_\adv(\mathcal{P} \setminus \mathcal{H}, \state), \mathsf{map}(\mathsf{BlindTx}_\adv(\state, \mathcal{P} \setminus \mathcal{H}), \buffer), \adv) \\
    \t \pcelse \\
    \t\t \mathsf{send}(\mathsf{Blind}(P, \state), \mathsf{map}(\mathsf{BlindTx}(\state, P), \buffer), P) \\
    \pcif I = (\mathsf{MANTAIN}, \mathsf{sid}) \\
    \t TODO \\
    \inputs := \inputs || (I, P, \tau_L)
}
\end{pcvstack}
\qquad
\qquad
...
\pseudocode{
    ...
    N \gets \mathsf{ExtendPolicy}(\state, \texttt{NxtBC}, \buffer) \\
    \assert N \neq \emptyset \\
    \state \gets \state || \mathsf{Blockify}(N) \\
    \pcfor \mathsf{BTX} \in \buffer \\
    \t \pcif \mathsf{Validate}(\mathsf{BTX}, \state) = 0 \\
    \t\t \buffer \gets \buffer \setminus \mathsf{BTX}
}
\vspace{1em}
\pseudocode{
    ...
    \pcif \mathsf{Validate}(\mathsf{BTX}, \state) = 1 \\
    \t \buffer \gets \buffer \cup \mathsf{BTX} \\
}
\end{pcvstack}
\end{pchstack}
}
...
\begin{pchstack}[boxed]
\pseudocode{
    \mathsf{ExtendPolicy}_P(\state, \texttt{NxtBC}, \buffer)  \:\: \\[0.1\baselineskip ][\hline]
    N \gets \emptyset \\
    \pcfor \mathsf{BTX} \in \texttt{NxtBC} \\
    \t \pcif P(\state, \texttt{tx}) = 1 \\
...
\subsection{DEX functionality}
\begin{center}
\fbox{%
\parbox{0.96\linewidth}{%
{\Large \textbf{Functionality} $\mathcal{G}_{\mathsf{DEX}}$}  \\[1em]
\smallskip
\textbf{Parameters:} Integers ???, Algorithms $\mathsf{Exchange}$, $\mathsf{Convert}$,  \\
\smallskip
\textbf{Clock-time:} \\
\smallskip
\textbf{Variables and initialization:} The functionality intializes $\texttt{c\_round} = 0$ \\
...
\pseudocode{
    \mathsf{Input}(I, P)  \:\: \\[0.1\baselineskip ][\hline]
    \tau_L \gets \mathcal{G}_\mathsf{CLOCK} \\
    \pcif I = (\mathsf{EXCHANGE}, \mathsf{sid}, \texttt{round}, \transcript_r) \\
    \t \mathsf{Exchange}(\mathsf{sid}, \texttt{round}, \transcript, P) \\
    \pcif I = (\mathsf{REFUND}, \mathsf{sid}, \texttt{round}, \transcript) \\
    \t \mathsf{Refund}(\mathsf{sid}, \texttt{round}, \transcript) \\
    \t TODO \\
    \inputs := \inputs || (I, P, \tau_L)
}
\vspace{1em}
\pseudocode{
    \mathsf{Refund}(\mathsf{sid}, \round, \transcript_r, P) \:\: \\[0.1\baselineskip ][\hline]
    \assert(\transcript[P] = \transcript_r) \\
    (\tx_0, \ledger_0, \ledger_1, \tx_1) := \parse \transcript_r \\
    \texttt{state}^{\ledger_0} \gets \mathcal{G}_\mathsf{ledger}^{\ledger_0}(\mathsf{READ}, \mathsf{sid}) \\
    \assert \tx_0 \in \texttt{state}^{\ledger_0} \\
}
\end{pcvstack}
\qquad
\qquad
\begin{pcvstack}
\pseudocode{
    \mathsf{Exchange}(\mathsf{sid}, \texttt{round}, \transcript_r, P) \:\: \\[0.1\baselineskip ][\hline]
    \assert \round = \cround[P]+ 1 \\
    \pcif \round = 1 \\
    \t (\tx_0, \ledger_0, \ledger_1, \dst_1) := \parse \transcript_r \\
    \t (\amnt_0, \src_0, \dst_0) := \parse \tx \\
    \t \amnt_1 := \mathsf{Convert}(\amnt_0, \ledger_0, \ledger_1) \\
    \t \assert \ledger_0, \ledger_1 \in \pool \land \amnt_1 \leq \mathsf{max}(\ledger_1) \land \dst = P_{\ledger_0} \\
    \t \tx_1 := (\amnt_1, P_{\ledger_1}, \dst_1) \\
    \t \send(\tx_1, P) \\
    \t \transcript[P] := \transcript_r || \tx_1 \\
    \pcif \round = 2 \\
    \t \sigma^0_{\transcript} := \parse \transcript_r \\
    \t \assert \mathcal{F}_\mathsf{SIG}(\texttt{Verify}, \mathsf{sid}, \transcript, \sigma_{\transcript}, \mathsf{VK}[P]) \\
    \t \sigma^1_{\transcript} \gets \mathcal{F}_\mathsf{SIG}(\texttt{Sign}, \mathsf{sid}, \transcript) \\
    \t \send(\sigma^1_{\transcript}, P) \\
    \t \transcript[P] := \transcript_r || \sigma^1_{\transcript} \\
    \t \pcwhile \tx_0 \notin \texttt{state}^{\ledger_0} \\
    \t\t \texttt{state}^{\ledger_0} \gets \mathcal{G}_\mathsf{ledger}^{\ledger_0}(\mathsf{READ}, \mathsf{sid}) \\
    \t \tx_1 := \tx_1 || W_{\ledger_1} \\
    \t\t \mathcal{G}_\mathsf{ledger}^{\ledger_1}(\mathsf{SUBMIT}, \mathsf{sid}, \tx_1) \\
    \cround[P] = \cround[P] + 1
}
\vspace{1em}
\pseudocode{
    \mathsf{Setup}(\mathsf{sid}) \:\: \\[0.1\baselineskip ][\hline]
}
\end{pcvstack}
\end{pchstack}
}
